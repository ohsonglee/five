...
/*
 *  콤마(,)로 구분된 문자열 데이터로부터(comma seperated value; csv)로부터 
 *  인스턴스 생성하기
 *  - 클래스 메서드 (static 메서드)의 활용 사례 공부  
 *  - 데이터항목이 추가되거나, 문자열의 표현 순서가 바뀌더라도 
 *    소스코드를 유지보수하기 쉽다. 
 *    
 */
public class StudentScore {
	 ...
	 */
	public static StudentScore fromCSV(String value){ //클래스메소드라면 this을 사용하지 않는게 맞다.
		String[] data = value.split(",");
		
		StudentScore obj = new StudentScore(data[0]); //숫자로 변환작업
